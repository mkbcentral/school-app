                    ...
                    <div class="d-flex justify-content-between">
                        <div class="d-flex">
                            <div class="form-group mr-2">
                                <x-label value="{{ __('Filtre par date') }}" />
                                <x-input class="" type='date' placeholder="Filtre par date"
                                    wire:model='date' />
                            </div>
                            <div class="form-group ">
                                <x-label value="{{ __('Filtre par mois') }}" />
                            ...
                            <tbody>
                                @foreach ($listPayment as $index => $payment)
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            @if ($isEditing == true && $idSeleted == $payment->id)
                                                <x-input class="" type='amount' placeholder="Montant"
                                                    wire:model='amount'
                                                    wire:keydown.enter='update({{ $payment->id }})' />
                                            @else
                                        ...
                                        <td class="text-center">
                                            <x-button
                                                wire:click.prevent='show({{ $payment }},{{ $payment->id }})'
                                            ...
                                            <x-button wire:click.prevent='delete({{ $payment->id }})'
